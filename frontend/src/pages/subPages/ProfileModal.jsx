    ...
    const load = async () => {
      ...
      try {
        const res = await fetch(
          "http://localhost:3000/api/profile/getComments",
          {
            method: "POST",
            credentials: "include",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ artId: post.id }),
          }
        );
  ...
  return (
    <div className="m-popup__scrim" onClick={onClose}>
      ...
      >
        ...
        {post.medium && (
          <p className="m-pQuickBio"><em>Medium: {post.medium}</em></p>
        )}
        {post.image && (
          <div
