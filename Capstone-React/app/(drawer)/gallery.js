...
const GalleryScreen = () => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      queryParams.append('nocache', '1');

      const res = await fetch(`${API_BASE}/gallery/categories?${queryParams.toString()}`, {
        method: 'GET',
        credentials: 'include',
        headers: {
  ...
  const fetchUserPreferences = async () => {
    try {
      ...
      if (res.ok) {
        ...
        catList.forEach((c) => {
          if (c.slug) {
            prefValues.push(c.slug);
          }
          if (c.name) {
            prefValues.push(c.name);
          }
        });
        if (prefValues.length > 0) {
          setUserPreferences(prefValues);
  ...
  const sortArtworksByPreference = (artworksToSort) => {
    ...
    if (preferenceTokens.size === 0) {
      return artworksToSort;
    }

    const getArtworkCategories = (artwork) => {
      const categoriesForArtwork = [];

      if (Array.isArray(artwork.categories)) {
        artwork.categories.forEach((c) => {
          if (typeof c === 'string' && c.trim().length > 0) {
            categoriesForArtwork.push(c);
          }
        });
      } else if (typeof artwork.categories === 'string' && artwork.categories.trim().length > 0) {
        try {
          const parsed = JSON.parse(artwork.categories);
          if (Array.isArray(parsed)) {
            ...
            categoriesForArtwork.push(parsed);
          }
        } catch {
          artwork.categories.split(',').forEach((c) => {
            if (c && c.trim().length > 0) {
              categoriesForArtwork.push(c.trim());
            }
          });
        }
      }

      if (typeof artwork.category === 'string' && artwork.category.trim().length > 0) {
        categoriesForArtwork.push(artwork.category);
      }

      return categoriesForArtwork;
    };

    const isPreferredArtwork = (artwork) => {
      const cats = getArtworkCategories(artwork);
      if (!cats.length) return false;
      return cats.some((cat) => {
        const token = typeof cat === 'string' ? cat.toLowerCase() : '';
        return token && preferenceTokens.has(token);
      });
    };

    const preferredArtworks = artworksToSort.filter((artwork) => isPreferredArtwork(artwork));
    const nonPreferredArtworks = artworksToSort.filter((artwork) => !isPreferredArtwork(artwork));
